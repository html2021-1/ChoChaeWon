$(document).ready(function () {
    ...
    $('.wavebtn').off('click').on('click', function () {
        // 변수선언 하단에서 위로 올려줌
        ...
        // 반복해서 클릭하지 못하도록 제어
        _btn = $(this);  //현재 클릭한 버튼
        _btn.hide();     // 다시 클릭하지 못하도록 숨긴다 => 화면전환이 완료되면 다시 보이게 함
    ...
    function transform(currentScene, nextScene, duration) {
        ...
        setTimeout(function () {
            ...
            $('#' + currentScene).stop().animate({opacity: 0}, 700, function () {
                ...
                // 추가 .wavebtn이 한번만 클릭할수 있도록 클릭하자마자 hide()로 숨기고 화면이 전환되면 다시 show()로 되돌려 놓는다 
                if (_btn.closest('.scene').attr('id') === 'space') _btn.show().prev('#heart').addBack().css('opacity', 0);
                else _btn.show();
            });
        }, duration);
    }
    ...
    $('.gnb ul li').children().on('click', function (e) {
        ...
        /* 애니메이션 무한 작동 */      
        if ($(this).parent().index() === 0) animationRepeat(8, '#castle img#magic', 'svg/2castle/MagicCastle_', 145);
